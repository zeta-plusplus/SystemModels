...
model StirlingEngine03_test01
  ...
  Modelica.Blocks.Sources.Ramp ramp1(duration = 10, height = 500, offset = 288.15 + 100, startTime = 10) annotation(
  ...
  Modelica.Mechanics.MultiBody.Joints.Revolute revolute1(n = {1, 0, 0}, useAxisFlange = true) annotation(
    Placement(visible = true, transformation(origin = {-120, -210}, extent = {{-10, 10}, {10, -10}}, rotation = 0)));
  ...
  Modelica.Mechanics.MultiBody.Parts.BodyCylinder bodyCylinder6(r = {0, 0.1, 0}) annotation(
    Placement(visible = true, transformation(origin = {30, -40}, extent = {{10, -10}, {-10, 10}}, rotation = -90)));
  Modelica.Mechanics.MultiBody.Joints.Prismatic prismatic2(n = {0, -1, 0}, s(fixed = false, start = 0.04), useAxisFlange = true) annotation(
    Placement(visible = true, transformation(origin = {30, -10}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  Modelica.Mechanics.MultiBody.Parts.FixedTranslation fixedTranslation4(animation = false, r = {0.35, 0.4 + 0.5, 0}) annotation(
    Placement(visible = true, transformation(origin = {-10, 10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Mechanics.Translational.Components.Mass mass1(L = 0.1, m = 1, s(fixed = false, start = 0.2)) annotation(
    Placement(visible = true, transformation(origin = {-70, 100}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  Modelica.Mechanics.Translational.Components.Mass mass2(L = 0.1, m = 1, s(fixed = false, start = 0.1)) annotation(
  ...
  Modelica.Mechanics.Rotational.Components.Inertia inertia1(J = 50, w(fixed = true, start = 10)) annotation(
    Placement(visible = true, transformation(origin = {-140, -240}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
equation
  connect(sweptVolume2.flange, mass2.flange_a) annotation(
    Line(points = {{70, 120}, {70, 120}, {70, 100}, {70, 100}}, color = {0, 127, 0}));
  connect(pipe1.port_b, sweptVolume2.ports[1]) annotation(
    Line(points = {{60, 160}, {70, 160}, {70, 140}}, color = {0, 127, 255}));
  connect(sweptVolume2.heatPort, thermalConductor2.port_b) annotation(
    Line(points = {{80, 130}, {80, 110}, {110, 110}}, color = {191, 0, 0}));
  connect(mass2.flange_b, prismatic2.axis) annotation(
    Line(points = {{70, 80}, {70, -18}, {36, -18}}, color = {0, 127, 0}));
  connect(inertia1.flange_b, revolute1.axis) annotation(
    Line(points = {{-130, -240}, {-120, -240}, {-120, -220}}));
  connect(revolute1.frame_b, bodyCylinder1.frame_a) annotation(
    Line(points = {{-110, -210}, {-100, -210}}, color = {95, 95, 95}));
  connect(world.frame_b, revolute1.frame_a) annotation(
    Line(points = {{-160, -210}, {-130, -210}}, color = {95, 95, 95}));
  ...
  annotation(
    Diagram(coordinateSystem(extent = {{-220, -260}, {200, 200}})),
    __OpenModelica_commandLineOptions = "",
    experiment(StartTime = 0, StopTime = 50, Tolerance = 1e-06, Interval = 0.1),
    __OpenModelica_simulationFlags(lv = "LOG_STATS", outputFormat = "mat", s = "dassl"));
end StirlingEngine03_test01;
